...
# --- SIDEBAR: IMPORTAÇÃO (Lógica Analítica) ---
with st.sidebar:
    ...
    if arquivo and st.button("🚀 Processar Dados"):
        # Scanner de mês simplificado
        m_final = 1 # Fallback para Janeiro se não detectar
        ...
        if tipo_dado == "Receita":
            ...
            for _, row in df.iterrows():
                ...
                if re.match(r'^\d', cod) and cod[-1] != '0': # Regra do último dígito
                    real = limpar_f(row.iloc[6])
                    if cod.startswith('9'): real = -abs(real) # Dedução
                    
                    # Pegamos a categoria existente ou padrão
            ...
            # Despesa (Lógica centavo a centavo)
...
with tab1:
    if not df_rec.empty:
        ...
        if not df_rf.empty:
            # CÁLCULOS SEM DUPLICAR PREVISÃO
            v_real = df_rf['realizado'].sum()
            m_ultima = max(ms_r)
            # Para o orçado/previsão, pegamos o valor único por código para não somar duplicado
            v_orc = df_rec[df_rec['mes'] == m_ultima].groupby('codigo_full')['orcado'].max().sum()
            v_prev = df_rf.groupby(['mes', 'codigo_full'])['previsao'].max().sum()
            ...
            # Agrupamos por mês, mas a previsão é o máximo do grupo para não explodir
            df_g = df_rf.groupby('mes').agg({'realizado': 'sum', 'previsao': 'sum'}).reset_index()
            # Correção manual da previsão agrupada para não somar o que é repetido
            for i, row in df_g.iterrows():
                df_g.at[i, 'previsao'] = df_rf[df_rf['mes'] == row['mes']].groupby('codigo_full')['previsao'].max().sum()
            ...
            # Painel de classificação movido para baixo para limpar o visual
...
with tab2:
    if not df_desp.empty:
        # Despesa mantida centavo a centavo
        f1, f2, f3 = st.columns(3)
