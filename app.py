...
with tab1:
    if not df_rec.empty:
        ...
        nat_sel = f3.multiselect("Natureza (Filtro Selecionável):", sorted(df_rec['natureza'].unique()))
        ...
        if not df_rf.empty:
            ...
            # Orçado consolidado (último mês selecionado para não duplicar orçamento fixo)
            m_ultima = max(ms_r)
            ...
            # Linha do Orçado
            fig.add_trace(go.Scatter(x=df_g['mes'].map(lambda x: MESES_NOMES[x-1]), y=df_g['orcado'], name="Orçado (Meta)", line=dict(color='#FF9800', width=3, dash='dot')))
            
            fig.update_layout(height=380, margin=dict(l=0, r=0, t=30, b=0), title="Realização vs Orçado por Mês", hovermode="x unified")
...
with tab2:
    if not df_desp.empty:
        # Mantendo a lógica de despesa centavo a centavo aprovada
        f1, f2, f3 = st.columns(3)
