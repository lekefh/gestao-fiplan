...
with st.sidebar:
    ...
    if not df_bkp.empty:
        ...
        st.download_button("📥 Baixar Backup (CSV)", data=csv, file_name="backup_fiplan_categorias.csv", mime="text/csv")
...
with tab1:
    if not df_rec.empty:
        ...
        if not df_rf.empty:
            ...
            # Cálculo de previsão/meta correta para o gráfico
            previsoes = []
            for m in df_g['mes']:
                previsoes.append(df_rf[df_rf['mes'] == m].groupby('codigo_full')['previsao'].max().sum())
            df_g['previsao'] = previsoes
