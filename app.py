...
with st.sidebar:
        ...
        if tipo_dado == "Receita":
            ...
            for _, row in df.iterrows():
                ...
                if re.match(r'^\d', cod) and cod[-1] != '0':
                    ...
                    # Preserva categoria se já existir para este código
            ...
            # Lógica de Despesa (Mensalização)
    ...
    if file_restore and st.button("🔄 Restaurar"):
        df_res = pd.read_csv(file_restore); conn = sqlite3.connect(DB_NAME); df_res.to_sql("receitas", conn, if_exists="replace", index=False); conn.commit(); conn.close()
...
with tab1:
    if not df_rec.empty:
        ...
        with st.expander("🏷️ Classificar Categorias de Receita"):
            ...
            sel_nat = c1.selectbox("Selecione a Natureza:", sorted(df_rec['natureza'].unique()), key="sel_nat_class")
        ...
        if not df_rf.empty:
            ...
            k1.metric("Orçado Atual", f"R$ {v_orc:,.2f}"); k2.metric("Realizado", f"R$ {v_real:,.2f}"); k3.metric("Atingimento", f"{(v_real/v_orc*100 if v_orc!=0 else 0):.1f}%")
