...
def extrair_mes_fiplan(arquivo, tipo):
    try:
        # Lê apenas as primeiras linhas para identificar o mês
        df_header = pd.read_excel(arquivo, nrows=10, header=None)
        texto_header = ""
        if tipo == "Receita":
            # Na Receita costuma estar na linha 5 ou 6
            texto_header = str(df_header.iloc[5, 0]).upper()
        else:
            # Na Despesa FIP 616 está na linha 5 (índice 5 do pandas)
            texto_header = str(df_header.iloc[5, 0]).upper()
        
        for nome, num in MESES_MAPA.items():
            if nome in texto_header:
                return num
    except:
        pass
    return None
...
with st.sidebar:
    ...
    mes_manual = st.selectbox("Mês (se não detectado)", range(1, 13), index=0, format_func=lambda x: MESES_NOMES[x-1])
    ano_ref = st.number_input("Ano", value=2026)
    
    if arquivo and st.button("🚀 Processar Dados"):
        # Tenta detectar o mês automaticamente
        mes_detectado = extrair_mes_fiplan(arquivo, tipo_dado)
        mes_final = mes_detectado if mes_detectado else mes_manual
        
        if mes_detectado:
            st.sidebar.success(f"Mês detectado: {MESES_NOMES[mes_detectado-1]}")
...
# --- CARGA E INICIALIZAÇÃO DE VARIÁVEIS DE CONFRONTO ---
conn = sqlite3.connect(DB_NAME)
...
v_real_rec, v_emp_d, v_liq_d, v_pag_d = 0.0, 0.0, 0.0, 0.0
...
with tab1:
    if not df_rec_inc.empty:
        ...
        if not df_rf.empty:
            ...
            st.metric("Realizado Acumulado", f"R$ {v_real_rec:,.2f}")
...
with tab2:
    if not df_desp_inc.empty:
        f1, f2 = st.columns(2)
        anos_d = f1.multiselect("Anos:", sorted(df_desp_inc['ano'].unique()), default=df_desp_inc['ano'].unique(), key="ad2")
        meses_d = f2.multiselect("Meses:", sorted(df_desp_inc['mes'].unique()), default=df_desp_inc['mes'].unique(), format_func=lambda x: MESES_NOMES[x-1], key="md2")
        ...
        if not df_df.empty:
            ...
            st.metric("Pago Acumulado", f"R$ {v_pag_d:,.2f}")
            st.dataframe(df_df[['natureza', 'empenhado', 'pago']].style.format('{:,.2f}'), use_container_width=True)

# --- ABA 3 ---
with tab3:
    st.subheader("⚖️ Confronto")
    c1, c2 = st.columns(2)
    c1.metric("Receita (Filtro)", f"R$ {v_real_rec:,.2f}")
    c2.metric("Despesa Paga (Filtro)", f"R$ {v_pag_d:,.2f}")
